...
import { ExtractionResult, Block, Line, Word, BoundingBox } from "@/types";
...
export default function PerfectExtractionDashboard({
  ...
  const getLayoutElements = (): LayoutElement[] => {
    ...
    if (page.lines && page.lines.length > 0) {
      const avgConfidence = page.lines.reduce((sum: number, l: Line) => sum + (l.words.reduce((wordSum: number, w: Word) => wordSum + w.confidence, 0) / l.words.length || 0), 0) / page.lines.length;
      elements.push({
        type: "Lines",
